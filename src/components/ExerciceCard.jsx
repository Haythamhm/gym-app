    ...
    <div className='p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap'>
        <div className='flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4'>
            ...
            <h2 className='capitalize whitespace-nowrap truncate max-w-full text-lg sm:text-xl md:text-2xl flex-1 sm:text-center'>
                {exercice.name.replaceAll("_", " ")}
            </h2>
            <p className='text-sm text-slate-400 capitalize'>{exercice.type}</p>
        </div>
        ...
        <div className='flex flex-col'>
            ...
            <p className='capitalize'>{exercice.muscles.join(' & ')}</p>
        </div>

        <div className='flex flex-col bg-slate-950 gap-2'>
            {exercice.description.split('___').map((val) =>{
                return (
        ...
        <div className='grid grid-flow-cols-2 sm:grid-cols-4 sm:place-items-center gap-2'>
            {['reps','rest','tempo'].map(info => {
                return (
                    <div key={info} className='flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900 w-full'>
                        <h3 className='capitalize text-slate-400 text-sm'>{info === 'reps' ? `${exercice.unit}` : info}</h3>
                        <p className='font-medium'>{exercice[info]}</p>
                    </div>
                )
            })}
